...
- 支持整页翻译：点击“翻译整个页面”会扫描当前页面正文并批量翻译，不再只按鼠标所在段落处理。
...
点击扩展图标 → “翻译整个页面”。译文会插入到原文段落下面；如果页面已经有整页译文，再次点击会清除译文。弹窗默认只保留常用入口：整页翻译、文档翻译、清除本页译文、设置/测试连接。

整页翻译会优先抓取 `p/li/blockquote/标题/表格单元格`，也会兼容常见的 `article/main/section/[role="main"]` 下 `div` 形式正文，适合 Inoreader、新闻站和很多现代网页。
